...
begin
  ...
  select jsonb_build_object(
    ...
    'activeSubscriptions', (select count(distinct user_id) from public.subscriptions where status = 'active'),
    'onTrial', (
      select count(*) from public.profiles p
      where p.deleted_at is null and p.trial_started_at is not null
        and now() < p.trial_started_at + (public.get_setting_int('trial_duration_minutes') || ' minutes')::interval
    ),
    'signupsLast30Days', (select count(*) from public.profiles where created_at > now() - interval '30 days'),
    'revenueLast30DaysCents', coalesce((select sum(amount_cents) from public.payments where paid_at > now() - interval '30 days'), 0),
    'paymentsLast30Days', (select count(*) from public.payments where paid_at > now() - interval '30 days'),
    'topCourses', coalesce((
...
-- ---------- 7. COBRO MANUAL: EXTENDER Y CORTAR ACCESO ----------

-- has_content_access con comprobación de current_period_end. Sin fecha
-- futura, un active NO da acceso: es lo que permite que "no renovar"
-- corte solo al llegar la fecha, sin cron.
create or replace function public.has_content_access(uid uuid)
returns boolean
language sql
stable
...
create or replace function public.admin_extend_access(
  target_user_id uuid, months integer default 1,
  amount_cents integer default null, note text default null
) returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if months is null or months < 1 or months > 24 then raise exception 'invalid_months' using errcode='check_violation'; end if;
  if not exists (select 1 from public.profiles where id = target_user_id and deleted_at is null)
    then raise exception 'user_not_found' using errcode='no_data_found'; end if;

  select id, current_period_end into existing
  ...
  if existing.id is not null then
    update public.subscriptions
       set status='active', current_period_end=new_end,
           past_due_since=null, canceled_at=null
     where id = existing.id;
  else
    insert into public.subscriptions (user_id, status, current_period_end)
      values (target_user_id, 'active', new_end) returning id into new_sub_id;
  end if;

  if amount_cents is not null and amount_cents > 0 then
    insert into public.payments (user_id, type, amount_cents, currency, external_payment_id, paid_at)
      values (target_user_id, 'recurring', amount_cents, 'EUR',
              'manual:' || gen_random_uuid()::text, now_ts)
      returning id into payment_id;
  end if;

  insert into public.audit_log (actor_id, action, entity_type, entity_id, diff)
  values (auth.uid(), 'access_extended', 'subscription',
          coalesce(existing.id, new_sub_id),
          jsonb_build_object(
            'user_id', target_user_id, 'months', months,
            'from', existing.current_period_end, 'to', new_end,
            'payment_id', payment_id, 'amount_cents', amount_cents, 'note', note));
...
revoke execute on function public.admin_extend_access(uuid, integer, integer, text) from public, anon;
grant execute on function public.admin_extend_access(uuid, integer, integer, text) to authenticated;
...
set search_path = public
as $$
declare now_ts timestamptz := now(); existing record;
begin
  if not public.is_admin() then raise exception 'forbidden' using errcode='insufficient_privilege'; end if;

  select id, current_period_end, status into existing
    from public.subscriptions where user_id = target_user_id
    order by created_at desc limit 1;
  if existing.id is null then raise exception 'no_subscription' using errcode='no_data_found'; end if;

  update public.subscriptions
     set status='canceled', canceled_at=now_ts,
         current_period_end=least(current_period_end, now_ts)
   where id = existing.id;

  insert into public.audit_log (actor_id, action, entity_type, entity_id, diff)
  values (auth.uid(), 'access_revoked', 'subscription', existing.id,
          jsonb_build_object(
...
begin
  ...
  from (
    select s.id as subscription_id, s.user_id, p.email, s.current_period_end,
      case when s.current_period_end < now() then 'expired' else 'upcoming' end as kind,
